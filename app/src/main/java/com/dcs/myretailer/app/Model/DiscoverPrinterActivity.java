...
public class DiscoverPrinterActivity extends Activity implements OnItemClickListener, OnCheckedChangeListener, Runnable {
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        ...
        Log.i("DFDF___","DFD_kprr_"+ KitchenPrinterActivity.binding.editTextProductName1.getText().toString());
        KitchenPrinterModel kpmodel = new KitchenPrinterModel();
        kpmodel.setOpenDeviceName(KitchenPrinterActivity.binding.editTextProductName1.getText().toString());
        kpmodel.setOpenDeviceMacAddress(item.get("Address"));

        try {
            Query.SaveKitchenPrinter(kpmodel);
        } catch (Exception e){
